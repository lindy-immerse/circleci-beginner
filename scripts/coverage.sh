...
LATEST_PIPELINE_ID=$(echo "$PIPELINES" | jq '.items' | jq '.[0]' | jq -r '.id')
LATEST_WORKFLOWS=$(curl -L "https://circleci.com/api/v2/pipeline/$LATEST_PIPELINE_ID/workflow?circle-token=b045cdea55d0e571a208181e4366f4c62c4016ce")
LATEST_WORKFLOW_ID=$(echo "$LATEST_WORKFLOWS" | jq '.items' | jq '.[0]' | jq -r '.id')
LATEST_JOBS=$(curl -L "https://circleci.com/api/v2/workflow/$LATEST_WORKFLOW_ID/job?circle-token=b045cdea55d0e571a208181e4366f4c62c4016ce")
JOB_NUMBER=$(echo "$LATEST_JOBS" | jq '.items' | jq '.[] | select(.name=="angular_testing")' | jq -r '.job_number')
